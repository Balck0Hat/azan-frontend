...
export default function SadaqaTracker({ hijriYear }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
      ...
      <p className="text-[var(--text-primary)] font-bold mb-3">🤲 متتبع الصدقات</p>
      <div className="text-center p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/15 mb-4">
        ...
        <p className="text-[var(--text-secondary)] text-xs mt-1">إجمالي الصدقات</p>
      </div>
      <div className="flex gap-2 mb-3">
        <input type="number" placeholder="المبلغ" value={amount} onChange={(e) => setAmount(e.target.value)}
          onKeyPress={(e) => e.key === 'Enter' && addEntry()}
          className="flex-1 bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl px-3 py-2 text-[var(--text-primary)] text-sm outline-none placeholder-[var(--text-muted)]" />
      ...
      <AnimatePresence>
        {entries.slice(0, 10).map((entry) => (
          <motion.div key={entry.id} initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }}
            ...
            className="flex items-center justify-between p-2.5 rounded-lg bg-[var(--bg-card)] border border-[var(--border-color)] mb-1.5">
            <div className="flex items-center gap-2">
              <span className="text-emerald-300 font-bold text-sm">{entry.amount.toLocaleString()}</span>
              <span className="text-[var(--text-muted)] text-xs">{entry.note}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-[var(--text-muted)] text-xs">{entry.date}</span>
