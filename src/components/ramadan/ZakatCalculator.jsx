...
export default function ZakatCalculator() {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
      ...
      <p className="text-white font-bold mb-4">💰 حاسبة زكاة الفطر</p>
      <div className="space-y-3 mb-4">
        <div>
          <label className="text-slate-400 text-xs block mb-1">البلد</label>
          <select value={country} onChange={(e) => setCountry(e.target.value)}
            className="w-full bg-slate-800/80 border border-white/10 rounded-xl px-3 py-2.5 text-white text-sm outline-none">
            {zakatFitrRates.map((r) => (<option key={r.country} value={r.country}>{r.country}</option>))}
          </select>
        </div>
        <div>
          <label className="text-slate-400 text-xs block mb-1">عدد الأفراد</label>
          <div className="flex items-center gap-3">
            <motion.button whileTap={{ scale: 0.85 }} onClick={() => setMembers((m) => Math.max(1, m - 1))}
              className="w-10 h-10 rounded-xl bg-white/[0.06] text-slate-300 font-bold border border-white/10">−</motion.button>
            <span className="text-white font-bold text-lg flex-1 text-center">{members}</span>
      ...
      <div className="p-4 rounded-xl bg-gradient-to-br from-emerald-500/10 to-indigo-500/10 border border-emerald-500/15 text-center">
        ...
        <p className="text-slate-400 text-xs mt-2">بمقدار صاع من {rate.unit} للفرد الواحد</p>
        <p className="text-slate-500 text-xs mt-1">تُخرج قبل صلاة العيد — ويجوز إخراجها من أول رمضان</p>
      </div>
    </motion.div>
  );
}
