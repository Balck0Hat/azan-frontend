...
export default function WeekChart({ weekStats, totalPrayed }) {
  ...
  return (
    <div className="mt-5 pt-5 border-t border-[var(--border-color)]">
      <h4 className="text-sm font-semibold text-[var(--text-secondary)] mb-4">إحصائيات الأسبوع</h4>
        ...
        {weekStats.map((day, idx) => {
          ...
          return (
            <div key={idx} className="flex-1 flex flex-col items-center gap-1.5">
              <span className="text-[10px] text-[var(--text-muted)] font-mono">{day.prayed}</span>
              <div className="w-full h-20 rounded-lg bg-[var(--bg-card)] relative overflow-hidden">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="rounded-xl bg-[var(--bg-card)] border border-[var(--border-color)] p-3 text-center">
          ...
          <div className="text-[10px] text-[var(--text-muted)] mt-0.5">صلاة هذا الأسبوع</div>
        </div>
        <div className="rounded-xl bg-[var(--bg-card)] border border-[var(--border-color)] p-3 text-center">
          <div className="text-xl font-bold text-purple-400">{commitmentPct}%</div>
          <div className="text-[10px] text-[var(--text-muted)] mt-0.5">نسبة الالتزام</div>
        </div>
      </div>
    </div>
  );
